...
-- Execute este script após criar as views
...
DECLARE
  ...
  horas_esperadas DECIMAL(5,2) := 8.0; -- Valor padrão
  turno_info RECORD;
BEGIN
...
DECLARE
  ...
  horas_planejadas_valor DECIMAL(6,2) := 176; -- Valor padrão
...
DROP TRIGGER IF EXISTS update_centro_custo_updated_at ON centro_custo;
CREATE TRIGGER update_centro_custo_updated_at BEFORE UPDATE ON centro_custo FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_funcionario_updated_at ON funcionario;
CREATE TRIGGER update_funcionario_updated_at BEFORE UPDATE ON funcionario FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_parametrizador_updated_at ON parametrizador;
CREATE TRIGGER update_parametrizador_updated_at BEFORE UPDATE ON parametrizador FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_planejamento_updated_at ON planejamento;
CREATE TRIGGER update_planejamento_updated_at BEFORE UPDATE ON planejamento FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
